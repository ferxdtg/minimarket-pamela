...
// 1. Hacemos las propiedades opcionales (?) para eliminar el error en page.tsx
...
export default function CartDrawer(props: CartDrawerProps) {
  ...
  // =========================================================================
  // ⚠️ AQUÍ DEBES RESTAURAR TU ESTADO GLOBAL DEL CARRITO ⚠️
  // Como tu page.tsx no envía props, debes volver a llamar a tu Hook Global.
  // Ejemplo si usabas Zustand o Context:
  // const { isOpen, closeCart, items, updateQuantity, clearCart } = useCartStore();
  // =========================================================================

  // Valores de respaldo (Borra esto y usa tu hook global si lo tienes)
  const isOpen = props.isOpen || false;
  const onClose = props.onClose || (() => {});
  const items = props.items || [];
  const onUpdateQuantity = props.onUpdateQuantity || (() => {});
  const onClearCart = props.onClearCart || (() => {});

  // Datos del Cliente y Pamela Coins
    ...
    <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md h-full shadow-2xl flex flex-col justify-between overflow-y-auto">
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 hover:bg-slate-200 transition cursor-pointer">✕</button>
        </div>
            ...
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-2 py-1 shadow-sm">
                    <button onClick={() => onUpdateQuantity(item.id, -1)} className="font-black text-slate-500 hover:text-red-600 w-5 text-center cursor-pointer">-</button>
              ...
              <div className="bg-gradient-to-r from-amber-500/15 via-amber-500/10 to-yellow-500/15 border border-amber-500/40 rounded-2xl p-3.5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-amber-500/20 border border-amber-500/50 flex items-center justify-center text-xl animate-bounce shrink-0 shadow-inner">🪙</div>
                  <div>
                ...
                <div className="text-right shrink-0">
                  <span className="bg-amber-500 text-slate-950 font-black text-xs px-2.5 py-1 rounded-xl shadow-sm inline-block">+{coinsEarned} Coins</span>
                </div>
              </div>
              ...
              <form onSubmit={handleCheckout} className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setOrderType("DELIVERY")} className={`py-2 rounded-xl text-xs font-bold transition border cursor-pointer ${orderType === "DELIVERY" ? "bg-red-600 text-white border-red-600 shadow-sm" : "bg-slate-50 text-slate-600 border-slate-200"}`}>🛵 Delivery (S/ 5.00)</button>
                  <button type="button" onClick={() => setOrderType("RECOJO")} className={`py-2 rounded-xl text-xs font-bold transition border cursor-pointer ${orderType === "RECOJO" ? "bg-red-600 text-white border-red-600 shadow-sm" : "bg-slate-50 text-slate-600 border-slate-200"}`}>🏪 Recojo en Tienda</button>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Nombre Completo</label>
                  <input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Ej. María Pérez" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 focus:outline-none focus:border-red-600" required />
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Número de Celular / WhatsApp</label>
                  <input type="tel" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} onBlur={handlePhoneBlur} placeholder="Ej. 950323959" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 focus:outline-none focus:border-red-600" required />
                </div>
                ...
                {clientPoints > 0 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3 space-y-2">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer pt-1 border-t border-amber-200/60">
                      <input type="checkbox" checked={useCoins} onChange={(e) => setUseCoins(e.target.checked)} className="accent-amber-500 w-4 h-4 rounded cursor-pointer" />
                ...
                {orderType === "DELIVERY" && (
                  <div>
                    ...
                    <input type="text" value={clientAddress} onChange={(e) => setClientAddress(e.target.value)} placeholder="Ej. Av. Los Pinos 204" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 focus:outline-none focus:border-red-600" required />
                  </div>
                )}

                <div className="bg-slate-50 border border-slate-200 p-3 rounded-2xl space-y-1 text-xs">
                  <div className="flex justify-between text-slate-600"><span>Subtotal:</span><span>S/ {subtotal.toFixed(2)}</span></div>
                  ...
                  <div className="flex justify-between font-black text-slate-900 pt-1 border-t border-slate-200 text-sm"><span>TOTAL A PAGAR:</span><span className="text-red-600">S/ {total.toFixed(2)}</span></div>
                </div>

                <button type="submit" disabled={loading} className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl transition shadow-lg shadow-emerald-600/20 cursor-pointer text-xs">

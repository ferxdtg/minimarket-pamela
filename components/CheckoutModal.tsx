...
export default function CheckoutModal({ cartItems, onClose, onSuccess }: any) {
  ...
  const [reference, setReference] = useState(''); // ✨ NUEVO: Referencia
  ...
  const handleConfirmOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🛡️ MANTENEMOS TU LÓGICA INTACTA DE TRANSACCIÓN PARA DESCONTAR STOCK
      await runTransaction(db, async (transaction) => {
        ...
        // 2. Descontar el stock si todo está bien
        for (const item of cartItems) {
          const productRef = doc(db, "products", String(item.id));
          const productSnap = await transaction.get(productRef);
          const currentStock = productSnap.data()?.stock || 0;
        ...
        transaction.set(orderRef, {
          client: name,
          phone: phone,
          // Guardamos dirección + referencia si es delivery
      ...
      onSuccess();
      alert("¡Pedido confirmado con éxito! 🎉 Prepararemos tu orden en breve.");
    } catch (error: any) {
        ...
        <form onSubmit={handleConfirmOrder} className="space-y-4 text-sm">
          ...
          {deliveryType === 'delivery' && (
            <div className="space-y-3 bg-zinc-950/50 p-3 rounded-xl border border-zinc-800/80">
              <div className="flex gap-2">
                <input required type="text" placeholder="Dirección exacta (Calle, Mz, Lote)" value={address} onChange={(e) => setAddress(e.target.value)} className="flex-1 bg-zinc-950 border border-zinc-800 p-3 rounded-xl text-white outline-none focus:border-red-600 transition" />
                <button type="button" onClick={getLocation} className="px-3 bg-zinc-800 hover:bg-zinc-700 rounded-xl text-lg transition border border-zinc-700" title="Usar mi GPS">📍</button>
              </div>
              {/* CAMPO DE REFERENCIA */}
              <input type="text" placeholder="Referencia (Ej: Frente a la canchita)" value={reference} onChange={(e) => setReference(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 p-3 rounded-xl text-white outline-none focus:border-red-600 transition" />
            </div>
          )}
          ...
          <div className="space-y-3 bg-zinc-950/50 p-3 rounded-xl border border-zinc-800/80">
            ...
            {/* Si elige Efectivo, pedir billete para dar vuelto */}

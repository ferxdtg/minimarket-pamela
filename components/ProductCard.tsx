...
export default function ProductCard({ product }: { product: any }) {
  ...
  // Initialize isFavorite from localStorage
  useEffect(() => {
    try {
      const saved = localStorage.getItem("pamela_favorites");
      if (saved) {
        const favIds: string[] = JSON.parse(saved);
        setIsFavorite(favIds.includes(String(productId)));
      }
    } catch {}
  }, [productId]);
  ...
  const toggleFavorite = (e: React.MouseEvent) => {
    ...
    try {
      ...
      localStorage.setItem("pamela_favorites", JSON.stringify(favIds));
    ...
    <>
      <div id={`product-${productId}`} className="group scroll-mt-32 bg-white rounded-2xl p-3 sm:p-4 flex flex-col justify-between shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] relative h-full">
        <div>
          <div className="relative w-full aspect-square bg-slate-50/80 rounded-xl overflow-hidden mb-3 flex items-center justify-center border border-slate-50">
            ...
            <button
              ...
              className="absolute top-2 right-2 w-8 h-8 bg-white/90 backdrop-blur shadow-sm rounded-full flex items-center justify-center text-slate-400 hover:text-red-500 hover:scale-110 active:scale-95 transition-all z-10 cursor-pointer"
            >
              {isFavorite ? "❤️" : "🤍"}
            </button>
            ...
            {product?.image && product.image !== "/placeholder.png" ? (
              ...
              <span className="text-4xl opacity-20" role="img" aria-label="Producto sin imagen">📦</span>
            )}
          ...
          <div className="flex items-end justify-between mt-auto mb-4">
            <div>
              {/* Only show crossed price if originalPrice field exists and is greater than current price */}
              {product?.isOnSale && originalPrice && originalPrice > rawPrice && (
        ...
        <div className="w-full mt-auto h-[38px] flex items-end">
          {localQty === 0 ? (
            <button
              onClick={(e) => { e.stopPropagation(); if (!isOut) setLocalQty(1); }}
              disabled={isOut}
            ...
            <div className="flex items-center justify-between w-full gap-2">
              <div className="flex items-center justify-between bg-slate-50 border border-slate-200/80 rounded-xl p-1 w-[55%] shadow-inner h-[38px]">
                <button onClick={handleDecrease} aria-label="Disminuir" className="w-7 h-full rounded-lg bg-white text-slate-600 font-black text-lg flex items-center justify-center shadow-sm hover:bg-slate-100 active:scale-95 transition-all cursor-pointer">-</button>
                <span className="text-slate-900 font-black text-sm px-1 select-none text-center">{localQty}</span>
                <button onClick={handleIncrease} disabled={localQty >= stockNumber} aria-label="Aumentar" className="w-7 h-full rounded-lg bg-slate-200 text-slate-700 font-black text-lg flex items-center justify-center hover:bg-slate-300 active:scale-95 disabled:opacity-50 transition-all cursor-pointer">+</button>
              </div>
      ...
      {/* NON-BLOCKING TOAST - top-right corner, pointer-events-none so it never blocks interaction */}
      {showToast && (
        <div className="fixed top-20 right-4 z-[99999] bg-white border border-emerald-200 shadow-2xl rounded-2xl px-4 py-3 flex items-center gap-3 animate-in slide-in-from-right-4 fade-in duration-300 pointer-events-none max-w-xs">
          <div className="w-8 h-8 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center text-lg font-bold shrink-0">✓</div>
          <div>
            <p className="text-xs font-black text-slate-900">¡Añadido al carrito!</p>
            <p className="text-[10px] text-slate-500">{product?.name}</p>
          </div>
        </div>
      )}
    </>

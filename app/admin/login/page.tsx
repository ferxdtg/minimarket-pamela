...
/** Setea una cookie de sesión sencilla para que el middleware la valide */
function setSessionCookie() {
  // Expira al cerrar el navegador (sin Max-Age) — igual que browserSessionPersistence
...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    const auth = getAuth();
    // Cerrar sesión firebase Y borrar cookie al entrar a login
    signOut(auth).catch(() => {});
    clearSessionCookie();
    ...
    try {
      ...
      if (newAttempts >= 3) {
        ...
        setTimeout(() => {
          ...
          window.location.href = `https://wa.me/${adminWA}?text=${encodeURIComponent('🚨 *ALERTA DE SEGURIDAD*\nIntentos de acceso al Panel de Minimarket Pamela. Sistema bloqueado temporalmente.')}`;
        }, 4000);
      } else {
        playSciFiSound('error');
        setStatus('error');
        // ✅ Mensaje amigable, sin datos técnicos de Firebase
        setMessage(`CREDENCIALES INCORRECTAS. Intentos restantes: ${3 - newAttempts}`);
        setUsuario(''); setPassword('');
        setTimeout(() => { setStatus('idle'); setMessage('ESPERANDO IDENTIFICACIÓN...'); }, 3500);
      }
    }
  };
        ...
        <div className="bg-[#0a0a0c]/80 backdrop-blur-3xl border border-red-900/30 rounded-[2rem] shadow-[0_0_80px_rgba(220,38,38,0.15),inset_0_0_40px_rgba(220,38,38,0.05)] pt-16 pb-12 px-8 sm:px-12 relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className={`mt-6 p-4 rounded-xl border flex items-center justify-center gap-3 transition-all duration-500 shadow-[inset_0_0_30px_rgba(0,0,0,0.8)] ${
              ...
              : status === 'success' || status === 'vault' ? 'bg-emerald-950/20 border-emerald-500/50 text-emerald-400 shadow-[0_0_30px_rgba(16,185,129,0.2)] scale-[1.02]'
            ...
            <div className="pt-6">
              <button
                ...
                aria-label="Abrir compuerta principal"
              >
